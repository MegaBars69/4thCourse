...
double func_f (double t, double x)
{
    ...
    double uxx = -(16 * M_PI * M_PI) * sin (4 * M_PI * x) * cos (2 * M_PI) ;
    double Pxp = -3 * M_PI * sin (3 * M_PI * x) / (1.5 + cos (3 * M_PI * x));
    return u + 2 * u * ux + u * u * Pxp + Cp / p - mui * uxx / p;
}

double func_f0 (double t, double x)
{
    return exp(t) * (cos (3*M_PI*x) + 1.5) * (1 + 4 * M_PI * cos (4 * M_PI * x) * cos (2 * M_PI * t)) - 3 * M_PI * sin(3 * M_PI * x)* exp (t) * cos (2 * M_PI * t) * sin (4 * M_PI * x);
}
