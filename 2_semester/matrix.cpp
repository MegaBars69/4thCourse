...
func_point Matrix::init_func_point(Point point)
{
    double x = point.x;
    double y = point.y;
    double G = 0.0, V1 = 0.0, V2 = 0.0;

    // Начальное состояние: покоящийся газ с плотностью rho0 = 1.0
    const double rho0 = 1.0;
    const double G0 = log(rho0);
    ...
    if (point.status == block_status::inner) {
     ...
     if (point.status == block_status::inlet_left) {
        if (ramp_steps == 0) {
            G = log(inlet_rho);
            V1 = inlet_velocity;
        } else {
        ...
        // Остальные границы (стенки, выходы) – покой
        G = G0;
        V1 = 0.0;
        V2 = 0.0;
    }
...
void Matrix::fill_matrix_G(Point point, std::vector<eigen_triplet_t>& triplets, eigen_vector_t& rhs)
{
    ...
    if (point.status == block_status::inlet_left) 
    {
        double factor = (ramp_steps == 0) ? 1.0 : std::min(1.0, double(step) / ramp_steps);
        double target_rho = 1.0 + (inlet_rho - 1.0) * factor;
        triplets.emplace_back(matr_str_num, m00.global_num, 1.0);
        rhs[matr_str_num] = log(target_rho);
        return;
    }

    // Выход (нижний или верхний): экстраполяция нулевого порядка (∂G/∂y = 0)
    if (point.status == block_status::outlet_bottom) {
        func_point m0R = get_func_point_check(point, m_0R);
        triplets.emplace_back(matr_str_num, m00.global_num, 1.0);
        triplets.emplace_back(matr_str_num, m0R.global_num, -1.0);
        rhs[matr_str_num] = 0.0;
        return;
    }

    func_point mR0, mL0, m0R, m0L;
    ...
    if (point.status == block_status::inner)
    {
        ...
        // Источниковый член удалён (Func_0 = 0)
        rhs[m00.global_num] = 4 * m00.G
                              - (tau / h_x) * (2 - m00.G) * (mR0.V1 - mL0.V1)
                              - (tau / h_y) * (2 - m00.G) * (m0R.V2 - m0L.V2)
                              + 0.0;   // вместо 4 * tau * Func_0(...)
    }
...
void Matrix::fill_matrix_V1(Point point, std::vector<eigen_triplet_t>& triplets, eigen_vector_t& rhs)
{
    ...
    if (point.status == block_status::inlet_left) {
        triplets.emplace_back(matr_str_num, m00.global_num, 1.0);
        rhs[matr_str_num] = inlet_velocity;
        return;
    }
    if (point.status == block_status::inlet_left) 
    {
        double factor = (ramp_steps == 0) ? 1.0 : std::min(1.0, double(step) / ramp_steps);
        triplets.emplace_back(matr_str_num, m00.global_num, 1.0);
        rhs[matr_str_num] = inlet_velocity * factor;
        return;
    }

    // Выход снизу: ∂u1/∂y = 0 (экстраполяция нулевого порядка)
    if (point.status == block_status::outlet_bottom) {
        func_point m0R = get_func_point_check(point, m_0R);
        triplets.emplace_back(matr_str_num, m00.global_num, 1.0);
        triplets.emplace_back(matr_str_num, m0R.global_num, -1.0);
    ...
    // Остальные границы – условие прилипания (V1 = 0)
    if (point.status != block_status::inner)
    {
        triplets.emplace_back(matr_str_num, m00.global_num, 1);
        rhs[matr_str_num] = 0;
    ...
    if (point.status == block_status::inner)
    {
        ...
        // Источниковый член удалён (Func_1 = 0)
        rhs[matr_str_num] = m00.HV1 / tau
                            + (1 / (6 * h_x)) * (m00.V1 * m00.V1) * (mR0.H - mL0.H)
                            + (1 / (4 * h_y)) * (m00.V1 * (m0R.HV2 - m0L.HV2))
                            - (gas.P(mR0.H) - gas.P(mL0.H)) * (1 / (2 * h_x))
                            + (mu / (12 * h_x * h_y)) * (mRR.V2 - mRL.V2 - mLR.V2 + mLL.V2)
                            + 0.0;   // вместо m00.H * Func_1(...)
    }
}
...
void Matrix::fill_matrix_V2(Point point, std::vector<eigen_triplet_t>& triplets, eigen_vector_t& rhs)
{
    ...
    // Выход снизу: ∂u2/∂y = 0 (u2(m00) = u2(m0R))
    if (point.status == block_status::outlet_bottom) {
        func_point m0R = get_func_point_check(point, m_0R);
        triplets.emplace_back(matr_str_num, m00.global_num, 1.0);
        triplets.emplace_back(matr_str_num, m0R.global_num, -1.0);
    ...
    if (point.status == block_status::inner)
    {
        ...
        // Источниковый член удалён (Func_2 = 0)
        rhs[matr_str_num] = m00.HV2 / tau
                            + (1 / (6 * h_y)) * (m00.V2 * m00.V2) * (m0R.H - m0L.H)
                            + (1 / (4 * h_x)) * (m00.V2 * (mR0.HV1 - mL0.HV1))
                            - (gas.P(m0R.H) - gas.P(m0L.H)) * (1 / (2 * h_y))
                            + (mu / (12 * h_x * h_y)) * (mRR.V1 - mRL.V1 - mLR.V1 + mLL.V1)
                            + 0.0;   // вместо m00.H * Func_2(...)
    }
}

bool Matrix::solve_to_steady(int max_steps, double tol_steady, double &time_steady)
{
    double prev_norm_G = 0.0;
    for (int step = 1; step <= max_steps; ++step) {
        this->step = step;
        // сохраняем старое G
        std::vector<double> old_G(Dim);
        for (int i = 0; i < Dim; ++i) old_G[i] = func_points[i].G;

        if (init_and_solve_G() == -1) { return false; }
        if (init_and_solve_V() == -1) { return false; }

        double norm_G = 0.0;
        for (int i = 0; i < Dim; ++i) {
            double diff = func_points[i].G - old_G[i];
            norm_G += diff * diff;
        }
        norm_G = sqrt(norm_G / Dim);

        if (step > 10 && fabs(norm_G - prev_norm_G) < tol_steady * prev_norm_G) {
            time_steady = step * mesh.tau;
            return true;
        }
        prev_norm_G = norm_G;
    }
