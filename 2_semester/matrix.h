...
public:
    ...
    double inlet_velocity;   // w
    double inlet_rho;        // плотность на входе
    int ramp_steps = 0;   
    ...
    bool solve_to_steady(int max_steps, double tol_steady, double &time_steady);
    
    Matrix(P_gas gas, Mesh mesh, double w, double rho_in,
           solver_type solver = solver_type::own) :
        gas(gas), mesh(mesh), solver(solver),
        inlet_velocity(w), inlet_rho(rho_in) {
        Dim = mesh.Dim;
