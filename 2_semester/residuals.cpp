...
double Matrix::calc_res_C1 (int mode, std::vector<double> other_res)
{
    double max = -1;
    int max_i = -1;
    if (!other_res.empty ())
    {
        for (int i = 0; i < Dim; i++ )
        {
            ...
            if (check_point.status != block_status::outer)
            {
                double res {};
                if (mode == g)
                res = fabs (solution_G[i] - other_res[i]);
                if (mode == v1)
                    res = fabs (solution_V1[i] - other_res[i]);
                if (mode == v2)
                    res = fabs (solution_V2[i] - other_res[i]);
                if (res > max)
                {
                    max = res;
                    max_i = i;
...
double Matrix::calc_res_L2 (int mode, std::vector<double> other_res)
{
    double sum = 0;
    if (!other_res.empty ())
    {
...
double Matrix::calc_res_W1 (int mode, std::vector<double> other_res)
{
    double sum = 0;
    ...
    {
        if (mode == g)
            prev_res = fabs (solution_G[0] - other_res[0]);
        if (mode == v1)
            prev_res = fabs (solution_V1[0] - other_res[0]);
        if (mode == v2)
            prev_res = fabs (solution_V2[0] - other_res[0]);
        for (int i = 1; i < Dim; i++)
        {
            double res {};
            Point check_point = mesh.mesh_points[i];
